...
class ComplaintService
{
    ...
    {
        // Ticket numbers are generated by reading the current maximum, so two
        // simultaneous complaints can derive the same one. The unique index is the
        // source of truth: on collision, retry so the second request picks up the
        // number the first one just committed.
        $attempts = 0;

        while (true) {
            try {
                return $this->createOnce($data, $files);
            } catch (UniqueConstraintViolationException $e) {
                if (!str_contains($e->getMessage(), 'ticket_number') || ++$attempts >= 5) {
                    throw $e;
                }
            }
        }
    ...
    private function createOnce(array $data, ?array $files = null): Complaint
    {
        DB::beginTransaction();
        try {
