...
<!-- Visitors -->
        ...
        <!-- Vehicles -->
        @if(Auth::user()->isSuperAdmin() || Auth::user()->hasPermission('vehicles.view'))
            <a href="{{ route('vehicles.index') }}"
               class="group flex items-center px-2 py-2 text-sm font-medium rounded-md {{ request()->routeIs('vehicles.*') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                <svg class="mr-3 h-6 w-6 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
...
<!-- Amenities (DROPDOWN) -->
        @if(Auth::user()->isSuperAdmin() || Auth::user()->isSocietyAdmin() || Auth::user()->isResident() || Auth::user()->isStaff())
            @php
                $amenityActive = request()->routeIs('amenities.*');
            @endphp

            <div x-data="{ open: {{ $amenityActive ? 'true' : 'false' }} }" class="space-y-1">
                <button type="button"
                        @click="open = !open"
                        class="w-full group flex items-center justify-between px-2 py-2 text-sm font-medium rounded-md
                        {{ $amenityActive ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                    <div class="flex items-center">
                        <svg class="mr-3 h-6 w-6 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                ...
                <div x-show="open" x-cloak class="ml-10 space-y-1">
                    <a href="{{ route('amenities.index') }}"
                       class="block px-2 py-2 text-sm rounded-md {{ request()->routeIs('amenities.index') || request()->routeIs('amenities.show') || request()->routeIs('amenities.create') || request()->routeIs('amenities.edit') ? 'text-emerald-400 font-semibold' : 'text-gray-300 hover:text-white' }}">
                        All Amenities
                    </a>

                    <a href="{{ route('amenities.my-bookings') }}"
                       class="block px-2 py-2 text-sm rounded-md {{ request()->routeIs('amenities.my-bookings') ? 'text-emerald-400 font-semibold' : 'text-gray-300 hover:text-white' }}">
                        My Bookings
                    </a>
                </div>
            </div>
        ...
        <!-- Divider + Management -->
        @if(Auth::user()->isSuperAdmin() || Auth::user()->isSocietyAdmin() || Auth::user()->hasPermission('flats.view'))
            @php
